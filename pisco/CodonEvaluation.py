...
def batch_get_CSI_value(dna_list, weights):
    # dna_list: List[str]
    # weights: dict
    return [CAI(dna, weights) for dna in dna_list]

def batch_get_cfd(dna_list, codon_frequencies, threshold=0.3):
    return [get_cfd(dna, codon_frequencies, threshold=threshold) for dna in dna_list]

def batch_get_GC_content(dna_list):
    return [get_GC_content(dna) for dna in dna_list]
